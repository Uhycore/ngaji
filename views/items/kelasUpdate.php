    ...
    <div class="flex">
        ...
        <div class="flex-1 p-20">
            ...
            <div class="max-w-lg mx-auto bg-gradient-to-br from-white to-gray-100 p-8 rounded-2xl shadow-xl transform hover:shadow-2xl transition-all duration-300">
                ...
                <form action="index.php?modul=kelas&fitur=update" method="POST" class="space-y-6">
                    <!-- Hidden input untuk ID Kelas -->
                    <input type="hidden" id="idKelas" name="idKelas" value="<?php echo htmlspecialchars($kelas['id']); ?>">

                    <!-- kodeKelas -->
                    <div>
                        <label for="kodeKelas" class="block text-gray-600 text-sm font-semibold mb-2">Kode Kelas:</label>
                        <span class="text-gray-700"><?php echo isset($kelas['id']) ? htmlspecialchars($kelas['id']) : ''; ?></span>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="guruId" name="guruId" class="shadow border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-300 ease-in-out hover:shadow-md">
                            <?php foreach ($gurus as $guru): ?>
                                <option value="<?php echo htmlspecialchars($guru['id']); ?>" <?php echo ($guru['id'] == $kelas['guruId']) ? 'selected' : ''; ?>>
